...
def request_to_dataframe(request: PredictionRequest, allowed_locations: set[str]) -> pd.DataFrame:
    """Turn a validated PredictionRequest into a single-row DataFrame with the
    exact column names the training pipeline expects. Unknown locations are
    mapped to 'other', mirroring the notebook's top-N grouping step."""
